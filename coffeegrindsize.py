...
# Roundness is defined as the ratio of cluster to all pixels in the smallest circle
# that encompasses all pixels of a cluster.
def_min_roundness = 0
...
reference_objects = {
    ...
    "Canadian Dime": 18.03,
    "Canadian Nickel": 21.3,
    #"Canadian Penny": None,  # production ceased in 2012.
    "US Dollar": 26.92,
    "US Quarter": 24.26,
    "US Dime": 17.91,
    "US Nickel": 21.21,
...
class coffeegrindsize_GUI:
    ...
    def __init__(self, root):
        ...
        # Remember the root object for the full user interface so that the methods of
        # coffeegrindsize_GUI can refer to it.
        self.master = root
        ...
        # That will contain option buttons and settings and the image.
        ...
        if def_pixel_scale is None:
            ...
            def_pix_len = def_pixel_scale * float(reference_objects["Custom"])

        # Length of the reference object.
        # Comment on March 3, 2019: To add support for simplified output, use argument
        # simple_data_entry=simple_data_entry.
        (
        ...
        # Value of fractional threshold in units of flux in the blue channel of the
        # image.
        self.threshold_var = self.label_entry(
            def_threshold, "Threshold:", "%", advanced=True, clear_on_click=True
        )
        ...
        # represents the fraction of thresholded pixels inside the smallest circle that
        # encompasses the farthest thresholded pixels in one cluster.
        ...
        # Threshold to select pixels dark enough to serve as a reference in the cost
        # function in the cluster breakup step.
        ...
        quick_checkbox = Checkbutton(
            self.frame_options, text="Quick & Approximate", variable=self.quick_var
        )
        ...
        #     self.frame_options, text="Zoom In", command=self.zoom_in_button
        # )
        # self.zoom_in_button.grid(
        #     row=self.options_row, column=0, columnspan=1, sticky=E
        # )

        # self.zoom_out_button = Button(
        #     self.frame_options, text="Zoom Out", command=self.zoom_out_button
        # )
        # self.zoom_out_button.grid(
        #     row=self.options_row, column=1, columnspan=1, sticky=W
        # )
        ...
        self.simple_zoom_in_button = Button(
            self.simple_frame_options, text="Zoom In", command=self.zoom_in_button
        )
        ...
        self.simple_zoom_out_button = Button(
            self.simple_frame_options, text="Zoom Out", command=self.zoom_out_button
        )
        ...
        self.simple_reset_zoom_button = Button(
            self.simple_frame_options, text="Reset View", command=self.reset_zoom
        )
        ...
        reset_params_button = Button(
            self.frame_options, text="Reset All Parameters", command=self.reset_status
        )
        ...
        separator_label = Label(
            self.frame_stats, text="", width=stats_colsep_width, bg=frame_stats_bg
        )
        ...
        # ey_average_entry = Label(
        #     self.frame_stats,
        #     textvariable=self.ey_average_var,
        #     width=stats_entry_width,
        #     bg=frame_stats_bg,
        # )
        ...
        # ey_stddev_entry = Label(
        #     self.frame_stats,
        #     textvariable=self.ey_stddev_var,
        #     width=stats_entry_width,
        #     bg=frame_stats_bg,
        # )
        ...
        # Quit button.
        quit_button = Button(
            toolbar2, text="Quit", command=self.quit_gui, highlightbackground=toolbar_bg
        )
        ...
        # subMenu.add_command(
        #     label="Toggle Advanced Options...", command=self.toggle_advanced_options
        # )
        subMenu.add_separator()
        ...
        # Always keep track of the mouse position (this is used for zooming toward the
        # cursor).
    ...
    def select_reference_object_mouse(self, event):
        ...
        # Update the user interface status.
        self.status_var.set(
            "Use your mouse to click on one side of the reference object... You will "
            "then need to click again to set up a measuring line..."
        )
    ...
    def select_region(self, event):
        ...
        # Update the user interface status.
        self.status_var.set(
            "Click on the image to draw a polygon enclosing the coffee grounds, then "
            "press q..."
        )
    ...
    def quit_region_select(self, event):
        ...
        if self.mouse_click_mode == "SELECT_REGION":
            ...
            if polysize < 3:
                ...
                # Update the user interface status.
                self.status_var.set(
                    "Analysis Region Did Not Have Enough Corners (at least 3 are "
                    "needed)..."
                )
            else:
    ...
    def change_reference_object(self, *args):
        ...
        self.physical_length_var.set(
            reference_objects[self.reference_object.get()]
        )
    ...
    def change_display_type(self, *args):
        ...
        if self.display_type.get() == threshold_image_display_name:
            if self.img_threshold is None:
                # Update the user interface status.
                self.status_var.set(
                    "Thresholded Image not Available Yet... Use Threshold Image "
                    "Button..."
                )
        ...
        if self.display_type.get() == outlines_image_display_name:
            if self.img_clusters is None:
                # Update the user interface status.
                self.status_var.set(
                    "Cluster Outlines Image not Available Yet... Use Launch Particle "
                    "Detection Button..."
                )
        ...
        if self.display_type.get() == histogram_image_display_name:
            if self.img_histogram is None:
                # Update the user interface status.
                self.status_var.set(
                    "Histogram Figure not Available Yet... Use Create Histogram "
                    "Figure Button..."
                )
        ...
        if self.polygon_alpha is not None:
            ...
            if npoly > 1:
                ...
                for i in range(npoly - 1):
                    ...
                    # Redraw line.
                    line_obj = self.image_canvas.create_line(
                        line_x_start, line_y_start, line_x_end, line_y_end, fill="green"
                    )

                    # I will need to figure out how to delete those when they go
    ...
    def release_mouse(self, event):
        ...
        if self.mouse_click_mode == "SELECT_REFERENCE_OBJECT":
            ...
            self.status_var.set(
                ...
                + " pixels... Now select its physical size with the library in the "
                + "Reference Object dropdown menu, or with a custom Reference Physical "
                + "Size..."
            )
        ...
        if self.mouse_click_mode == "SELECT_REFERENCE_OBJECT_READY":
            ...
            # Refresh the user interface status.
            self.status_var.set(
                "Drag the red line across the reference object then click again... Be "
                "careful to exclude shadows, and make sure that the Angle of Reference "
                "Line entry is satisfactory to you..."
            )
    ...
    def move_start(self, event):
        ...
        if self.erase_clusters_mode is True:
            # 2019 March 3: Maybe this should be set in the "self.mouse_click_mode"
            # variable.
            ...
            # Determine cursor position on original image coordinates
            # (x,y -> alpha, beta).
    ...
    def downsample_image(self):
        ...
        self.img_source = self.img_source.resize(
            (int(float(self.img.size[0]) / 2), int(float(self.img.size[1]) / 2)),
            Image.ANTIALIAS,
        )
        self.img = self.img_source
        ...
        # Update the user interface status.
        self.status_var.set(
            "The Image was Downsampled by a Factor two... Current Size is ("
            + str(self.img.size[0])
            + ", "
            + str(self.img.size[1])
            + ")"
    ...
    def open_image(self, event):
        ...
        # image_filename = (
        #     "/Users/gagne/Documents/Postdoc/Coffee_Stuff/Grind_Size/Forte_half_seasoned/forte_3y_mid.png" # )
    ...
    def threshold_image(self, event):
        ...
        # If an analysis polygon is set, select only pixels inside the polygon to
    ...
    def launch_psd(self, event):
        ...
        # Sort the thresholded pixel indices by increasing brightness in the blue
        # channel.
        ...
        for i in range(nmask):
            ...
            qc_indices = self.quick_cluster(
                X_mask[ipreclust], Y_mask[ipreclust], X_mask[icurrent], Y_mask[icurrent]
            )
            ...
            # Unless the quick option is selected, now do a rejection of pixels based
            # on the image values in the blue channel. The goal of this step is to
            # break clumps of coffee grounds.
            if self.quick_var.get() == 1:
                ...
                # Order the cluster pixels w-r-t their distance from the current
                # starting pixel.
                ...
                if icurrent_in_clust[0].size != 1:
                    raise ValueError(
                        "The starting pixel was not found in a cluster. This should "
                        "never happen!"
                    )

                # Create a cost function along the positions of this cluster for pixel
                # rejection.
                cost = (
                ...
                # Loop on the cluster and check if the cost along the path to the
                # starting pixel is exceeded.
                ...
                for ci in range(iclust.size):
                    ...
                    # Make a list of all current pixels that are almost as dark as the
                    # reference pixel.
                    ...
                    if idark.size == 0:
                        raise ValueError(
                            "There should be at least one dark pixel in the cluster, "
                    ...
                    # If the current pixel was picked as a reference dark pixel then it
                    # is automatically accepted.
                    if iclust[ci] == idark:
                        maxcost_along_path[ci] = 0.0
                    else:
                        # Calculate distance between all points and a line that passes
                        # through pixel i and the reference dark pixel.
                        x1 = X_mask[iclust[ci]]
                        y1 = Y_mask[iclust[ci]]
                        x2 = X_mask[idark]
                        y2 = Y_mask[idark]
                        ...
                        # Skip this pixel if the path is broken (this means the line
                        # to the darkest pixel ventures outside the cluster).
            ...
            if self.mask_threshold_edge is not None:
                # Loop on edge pixels to determine if at least one is included in the
                # current cluster.
            ...
            # Determine roundness from the ratio of thresholded pixels in the circle
            # to the ratio of total pixels.
            if surface == 1:
                roundness = 1
            else:
            ...
            # Suppose that the hidden axis is equal to the short axis and calculate
            # volume.
    ...
    def refresh_cluster_data(self):
        ...
        for i in range(self.nclusters):
            ...
            for l in range(xlist.size):
                # Count neighbors.
                ineigh = np.where(
                    (np.abs(xlist - xlist[l]) <= 1) & (np.abs(ylist - ylist[l]) <= 1)
                )
    ...
    def quick_cluster(self, xlist, ylist, xstart, ystart):
        # self.mask_threshold[0][ipreclust],
        # self.mask_threshold[1][ipreclust],
        # self.mask_threshold[0][icurrent],
        # self.mask_threshold[1][icurrent]
        ...
        for i in range(xlist.size):
            ...
            if isel[0].size == 0:
                # If this was the only pixel that currently needed to be checked break
                # the loop.
                if xcheck.size == 1:
                    break
    ...
    def psd_hist_from_data(
        ...
        # If Y data is the mass (proportional to volume because we assume all
        # particles have the same mass density).
        ...
        # Loop on histogram bins and, within each bin, determine the average weight.
        # Use this average weight to reverse determine a number of objects and
        # estimate a Poisson error bar.
        ...
        avg_min_eneg = np.interp(
            p1s, np.flip(ycdfneg, axis=0), np.flip(xpdf[ileft[0]], axis=0)
        )
    ...
    def update_statistics(self):
        ...
        eys_stddev = (
            self.weighted_stddev(eys, weights, frequency=True, unbiased=True) * 100
        )
    ...
    def erase_clusters(self, event):
        ...
        if self.nclusters is None:
            # Update the user interface status.
            self.status_var.set(
                "Coffee Particles not Detected Yet... Use Launch Particle Detection "
                "Button..."
            )
        ...
        if self.display_type.get() != outlines_image_display_name:
            # Update the user interface status.
            self.status_var.set(
                "Please select 'Display Type' = 'Cluster Outlines' to use the 'Erase "
                "Clusters' tool."
            )
        ...
        if self.erase_clusters_mode is False:
            # Update the user interface status.
            self.status_var.set(
                "Entered 'Erase Clusters' mode. Click with the mouse to erase all "
                "clusters within the circle. Zoom in or out to set precision. Hit "
                "Escape or the 'Erase Clusters' button to end."
            )
    ...
    def create_histogram(self, event):
        ...
        if self.pixel_scale_var.get() == "None":
            # Update the user interface status.
            self.status_var.set(
                "The Pixel Scale Has Not Been Defined Yet... Use the Left Mouse Button "
                "to Draw a Line on a Reference Object and Choose a Reference Length in "
                "mm..."
            )
    ...
    def launch_help(self):
        webbrowser.open(
            # TODO: better PDF location OR replace with web-page OR replace with some
            #   other structured text (e.g. markdown).
            "https://www.dropbox.com/s/m2af0aer2e17xie/coffee_grind_size_manual.pdf?dl=0"
            # "https://github.com/jgagneastro/coffeegrindsize/raw/master/"
...
if __name__ == "__main__":
    ...
    # Refresh user interface in a try statement to avoid UTF-8 crashes when the user
    # interface tries to interpret unrecognized inputs like an Apple trackpad.
    while True:
        try:
            root.mainloop()
            break
